 ...
 var municipios_global = JSON.parse($("#dados_municipio").html());
 console.log(">>> " + municipios_global[0].pocos);
...
function setCardInfo (position, data) {
    ...
    let municipio = municipios_global;
    console.log(position + " >> " + municipio[position].id);
    $("#card-nome").html(municipio[position].nome);
    $("#card-pocos").html(municipio[position].pocos);
...
function idToPosition(id, data) {
	var municipio = municipios_global;
    for (var i in municipio) {
        if (municipio[i].id === id) {
            return Number(i);
        }
    }
}
...
function iegmClass (cl) {
    if(cl !== 0) return "temPoco";
    else return "iegmCM";

}

function changeMapColor(regiaoTipo, data, all, ind){
	var municipios = municipios_global;
    var allChosenArea =  new Array();
  
    for(var i = 0; i < municipios.length; i++)
        allChosenArea.push(municipios[i].id);

    for (var i of allChosenArea) {
        var pos = idToPosition(Number(i), data)
        var tempStr = municipios[pos].id;
        $("#" + regiaoTipo + "-" + tempStr).addClass(iegmClass(municipios[pos].pocos));
    }
}
...
function initialize() {
	showMap("-18.8800397", "-47.05878999999999", 1);        
}

function showMap(latitude, longitude, zoom) {	
  var mapDiv = document.getElementById("paraiba_mapa");
